...
class Graph:
    ...
    def addNode(
        ...
        if type(label) == tuple:
            node = self.nodes[label[1]] = self.nodeClass(label[0])
        else:
            node = self.nodes[label] = self.nodeClass(label)
        return node
    ...
    def firstKey(self) -> Type[BaseNode]:
        return next(iter(self.nodes.keys()), None)

    def firstValue(self) -> Type[BaseNode]:
    ...
    @staticmethod
    def hasCycle(nodes: List[Type[BaseNode]]) -> bool:
