...
class Genetic:
    ...
    def solve(self) -> Tuple[np.ndarray, int]:
        for gen in range(self.maxNumOfGenerations - 1):
            ...
            stop = np.where(fitness == 0)[0]

            if stop.shape[0]:
                return self.population[stop], gen + 1
            self.__log(
                f'Generacja: {gen + 1}',
                f'Najlepsze dopasowanie: {fitness.min()}'
            )

            self.createNewGeneration(fitness)

        fitness = self.fitFunction(self.population, self.constants)
        final = np.where(fitness == 0)[0]

        self.__log(
            f'Gen: {self.maxNumOfGenerations}',
            f'Best fit: {fitness.min()}'
        )

        if final.shape[0]:
            return self.population[final], self.maxNumOfGenerations
        self.__log("Nie znaleziono rozwiązania przy narzuconym limicie "
                   "generacji.")
        return np.array([self.population[np.argmin(fitness)]]), -1
